...
const PropertiesDialog = new Lang.Class({
	...
	this._source = new Gtk.Label({ label: _("Source: "), //Label for Source item in Properties dialog
	    ...
	    this._titleData.connect("changed", Lang.bind (this, this._setEditedName, this._docId));
        } else {
	...
	if (doc instanceof Documents.LocalDocument ){
	    this._sourceData = new Gtk.LinkButton({ label: this._directoryMetadata,
						    uri: this._sourcePath.get_uri(),
					            halign: Gtk.Align.START });
	}
	else
    ...
    _setEditedName: function(_newTitle, _docId) {
	this._newTitle = this._titleData.get_text();
	TrackerUtils.setEditedName(this._newTitle, _docId, null); 
    }
});
